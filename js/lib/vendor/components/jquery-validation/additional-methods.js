...
$.validator.addMethod("phones", function(phone_number, element) {
	var phones = /^((\+?86)|(\(\+86\)))?1\d{10}$/;
	phone_number = phone_number.replace(/\s+/g, "");
	phone_number = phone_number.replace(/，/g, ",");
	if(phone_number.indexOf(",")>=0){
		var valueArr = phone_number.split(","),
			isTrue = false;
		for(var i in valueArr){
			isTrue = this.optional(element) || phones.test(valueArr[i]); 
			if(!isTrue){
				return false;
			}
		}
		...
		return this.optional(element) || phones.test(phone_number); 
	}
}, "请正确填写您的手机号码");
...
$.validator.addMethod("tel", function(value, element) {
	...
	if(value.indexOf(",")>=0){
		var valueArr = value.split(","),
			isTrue = false;
		for(var i in valueArr){
			isTrue = this.optional(element) || tel.test(valueArr[i]); 
