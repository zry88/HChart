...
define(function() {
    ...
    var Framework7 = function(params) {
        ...
        app.getScroller = function(content) {
            var contentEl = content ? (content instanceof jQuery ? content : $(content)) : '';
            if (contentEl) {
                return contentEl[0].scroller;
            } else {
        ...
        app.popover = function(modal, target, removeOnClose) {
            ...
            if (typeof modal === 'string' && modal.indexOf('<') >= 0) {
                ...
                if (_modal.childNodes.length > 0) {
                    ...
                    if (removeOnClose) modal.add('remove-on-close');
                    $('body').append(modal);
                } else return false; //nothing found
            }
            modal = modal instanceof jQuery ? modal : $(modal);
            target = target instanceof jQuery ? target : $(target);
            if (modal instanceof jQuery && removeOnClose) modal.addClass('remove-on-close');
